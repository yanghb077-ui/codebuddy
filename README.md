...
3. **训练页面**：
   ...
   - 每组完成后点击“完成”保存并标记状态
   - 支持删除已保存组数
4. **动作库**：内置24个常见健身动作，分类清晰
5. **日历详情**：30天训练日历，点击日期查看详细训练记录
6. **动作分析**：单个动作历史与进步分析（训练量、重量、组数/次数趋势）
7. **综合分析**：训练部位分布、训练频率与强度趋势
8. **数据持久化**：所有训练数据存储在MongoDB数据库中
...
- **Axios**：HTTP客户端
- **Chart.js + react-chartjs-2**：训练数据图表
...
#### 删除训练记录中的某组
- **URL**: `DELETE /api/workouts/:id/sets`
- **功能**: 删除某组训练记录
- **参数**:
   - `id` - 训练记录ID
- **查询参数**:
...
-
#### 动作历史与数据分析
- **URL**: `GET /api/workouts/exercise-history/:exerciseId`
- **功能**: 获取单个动作历史详情与分析
- **参数**: `exerciseId` - 动作ID
- **查询参数**:
   - `days` - 查询天数范围
